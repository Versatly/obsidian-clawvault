...
export function registerCommands(plugin: ClawVaultPlugin): void {
	...
	plugin.addCommand({
		...
		name: "Setup graph colors (BrainMeld style)",
		callback: () => {
...
/**
 * Setup BrainMeld-style graph colors
...
async function setupGraphColors(plugin: ClawVaultPlugin): Promise<void> {
	try {
		...
		const snippetCSS = `/* ClawVault Graph Colors — BrainMeld Style */
...
`;
		...
		new Notice("🐘 Graph colors configured! Open Graph View to see the BrainMeld style.");
	} catch (error) {
